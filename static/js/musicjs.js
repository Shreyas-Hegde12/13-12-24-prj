...
function updateSlider(action) {
    if (action == 'reset') { slider.value = audioElement.currentTime; slider.max = audioElement.duration; }

    if (action == 'update') { audioElement.currentTime = slider.value; manualMove = false; }
...
async function fetchUrl(videoid) {
    const response = await fetch('/songurl', {
        ...
        setTimeout(() => { updateSlider('reset'); }, 1e3);
    }).catch(error => {
        console.error("Error fetching data:", error);
    });
