...
function togglePlay() {
    ...
    if (!songFetched | loading) { 
        playButton.innerHTML = "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 200 200'><radialGradient id='a5' cx='.66' fx='.66' cy='.3125' fy='.3125' gradientTransform='scale(1.5)'><stop offset='0' stop-color='#000'></stop><stop offset='.3' stop-color='#000' stop-opacity='.9'></stop><stop offset='.6' stop-color='#000' stop-opacity='.6'></stop><stop offset='.8' stop-color='#000' stop-opacity='.3'></stop><stop offset='1' stop-color='#000' stop-opacity='0'></stop></radialGradient><circle transform-origin='center' fill='none' stroke='url(#a5)' stroke-width='15' stroke-linecap='round' stroke-dasharray='200 1000' stroke-dashoffset='0' cx='100' cy='100' r='70'><animateTransform type='rotate' attributeName='transform' calcMode='spline' dur='2' values='360;0' keyTimes='0;1' keySplines='0 0 1 1' repeatCount='indefinite'></animateTransform></circle><circle transform-origin='center' fill='none' opacity='.2' stroke='#fff' stroke-width='15' stroke-linecap='round' cx='100' cy='100' r='70'></circle></svg>";
        loading = true;
        return;
    }
...
function updateSlider(bool) {
    ...
    if (bool === 0) {slider.value = audioElement.currentTime; slider.max = audioElement.duration;}

    if (bool === 1) {manualMove = true;}

    audioElement.currentTime = slider.value;

    if (bool === 1) {manualMove = false;}
    ...
    manualMove = true; 
        ...
        function setSong(data) {
            if (data) {
                ...
                if (song.title.length > 23) {song.title = song.title.slice(0, 22) + '..';}
                if (song.artist.length > 20) {song.artist = song.artist.slice(0, 17) + '..'; }
            });
            }
